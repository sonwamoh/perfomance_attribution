...
stocks_to_weights_idx = [
    ("MARUTI.BSE", 0.3),
    ("HINDUNILVR.BSE", 0.4),
    ("TATAMOTORS.BSE", 0.3),
]
stocks_to_sectors_idx = [
    ("MARUTI.BSE", "AUTO"),
    ("HINDUNILVR.BSE", "FMCG"),
    ("TATAMOTORS.BSE", "AUTO"),
]
...
class TestBuildPortfolio:
    def test_build_portfolio(self):
        ...
        test_portfolio = test_portfolio.sort_values(by=["symbol", "date"]).reset_index(
            drop=True
        )
    ...
    def test_incorrect_weight(self):
        ...
        with pytest.raises(pf.InvalidWeightException):
            pf.build_portfolio(
                1000000,
                stocks_to_weights_port_incorrect,
                datetime.datetime(2023, 1, 1),
                datetime.datetime(2023, 1, 4),
            )
    ...
    def test_build_index(self):
        ...
        test_index = test_index.sort_values(by=["symbol", "date"]).reset_index(
            drop=True
        )
...
class TestAttribution:
    def test_attribution_1(self):
        port_idx_attr = pf.perform_portfolio_attribution(
            port, idx, stocks_to_sectors_port, stocks_to_sectors_idx
        )
